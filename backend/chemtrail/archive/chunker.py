...
"""Video chunking logic for chemtrail webcam feeds."""
...
# Extended video format support for chemtrail webcams
...
def chunk_video(
    ...
    Args:
        video_path: Path to the input video file or RTSP/MJPEG stream URL.
        chunk_duration: Duration of each chunk in seconds.
        overlap: Overlap between consecutive chunks in seconds.
        continuous_mode: If True, continuously chunk a stream (yields chunks).
        output_dir: Directory for chunk files (temp dir if None).

    Returns:
        List of dicts with keys: chunk_path, source_file, start_time, end_time.
        In continuous_mode, yields chunks as they're recorded.
    ...
    tmp_dir = tempfile.mkdtemp(prefix="chemtrail_")
...
def is_still_frame_chunk(
    ...
    try:
        ...
        tmp_dir = tempfile.mkdtemp(prefix="chemtrail_still_")
